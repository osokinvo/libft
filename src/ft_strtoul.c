...
/*   Updated: 2020/02/15 15:01:33 by ghusk            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void				ft_strtoulunity(const char *nptr, char **end,
							unsigned int base)
{
	ft_base_is(&base);
	g_cutlim = (unsigned int)(g_cutoff % (unsigned long)base);
	g_cutoff = ULONG_MAX / base;
...
static unsigned long	ft_strtoulneg(const char *nptr, char **end,
						unsigned int base)
{
	g_str++;
	g_cutoff = ULONG_MAX;
	ft_strtoulunity(nptr, end, base);
	if (g_any)
		return (-g_result);
	else
		return (ULONG_MAX);
...
unsigned long			ft_strtoul(const char *nptr, char **end,
						unsigned int base)
{
	g_str = nptr;
	g_any = -1;
	...
	while (ft_isspace(*g_str))
		g_str++;
	if (*g_str == '-')
		return (ft_strtoulneg(nptr, end, base));
	else if (*g_str == '+')
		g_str++;
	g_cutoff = ULONG_MAX;
	ft_strtoulunity(nptr, end, base);
